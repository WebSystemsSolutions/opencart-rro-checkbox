...
class ModelExtensionPaymentCheckbox extends Model
{
    ...
    public function getBearToken()
    {
        ...
        $this->access_token = !empty($response['access_token']) ? $response['access_token'] : '';
    }

    public function connect()
    {
        $cashbox_key = $this->cashbox_key;
        $header_params = array('cashbox_key' => $cashbox_key);
        return $this->makePostRequest('/api/v1/shifts', array(), $header_params);
    }
    ...
    public function getCurrentCashboxInfo()
    {
        ...
        $header_params = array('cashbox_key' => $this->cashbox_key);
        return $this->makeGetRequest($url, array(), $header_params);
    }
    ...
    public function getReportText($report_id)
    {
        ...
        return $this->makeGetRequest($url, array(), array(), true);
    }
    ...
    private function makePostRequest($route, $params = array(), $header_params = array())
    {
        ...
        $header = array_merge($header, array('X-Client-Name: shhygolvv_khm'));
        $header = array_merge($header, array('X-Client-Version: 1'));
        ...
        return isset($response) ? (array)json_decode($response) : array();
    }

    private function makeGetRequest($route, $params = array(), $header_params = array(), $echo = false)
    {
        $url_host = $this->is_dev ? 'https://dev-api.checkbox.in.ua' : 'https://api.checkbox.in.ua';
        $url = $url_host . $route;
